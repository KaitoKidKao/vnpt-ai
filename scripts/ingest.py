...
def main():
    ...
    parser.add_argument(
        "--dir",
        action="append",
        help="Directory containing files to ingest (can be used multiple times)",
    )
        ...
        for d in args.dir:
            ...
            if dir_path.is_dir():
                for ext in ["*.json", "*.pdf", "*.docx", "*.txt"]:
                    ...
                    file_paths.extend(dir_path.rglob(ext))
            else:
    ...
    file_paths = sorted({Path(p).resolve() for p in file_paths})
